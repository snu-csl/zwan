...
// [JS] WALTZ specific variables
DEFINE_int32(waltz_test_type, 0, "WALTZ test type");
DEFINE_double(waltz_zipf_dist, 0.99, "WALTZ zipf distribution, default 0.99");
...
struct SharedState {
  ...
  // [JS] WALTZ zipfian generator related variables
  std::vector<uint64_t> key_seq; // key shuffler
  uint64_t items, base, num_zeta;
  double zipf_dist, alpha, zeta_n, eta, theta, zeta_2;
  std::mt19937_64 gen;
  std::mutex zipf_guard;

  double zeta(uint64_t st, uint64_t n, double last_zeta) {
    num_zeta = n;
    return zetastatic(st, n, last_zeta);
  };
  double zetastatic(uint64_t st, uint64_t n, double last_zeta) {
    double cur_zeta = last_zeta;
    for (uint64_t i = st; i < n; i ++) {
      cur_zeta += 1 / pow(i + 1, theta);
    }
    return cur_zeta;
  };
  uint64_t get_zipf(void) {
    std::lock_guard<std::mutex> lock(zipf_guard);

    if (items != num_zeta) {
      if (items > num_zeta) {
        // maybe not happened, requested zipf range is larger than init
        zeta_n = zeta(num_zeta, items, zeta_n);
        eta = (1 - pow(2.0 / items, 1 - theta)) / (1 - zeta_2 / zeta_n);
      }
    }

    double u = (double)(gen() % RAND_MAX) / ((double)RAND_MAX);
    double uz = u * zeta_n;
    if (uz < 1.0) {
      return base;
    }
    ...
    return base + (uint64_t)(items * pow(eta * u - eta + 1, alpha));
  }
  uint64_t get_key() {
    return key_seq[get_zipf()];
  }

  SharedState() : cv(&mu), perf_level(FLAGS_perf_level), gen(FLAGS_seed? FLAGS_seed : 1000) { 
    // [JS] Initialize Zipfian Generator at constructor
    // fill & shuffle key sequence vector
    key_seq.reserve(FLAGS_waltz_key_range);
    for (uint64_t i = 0; i < FLAGS_waltz_key_range; i ++) {
      key_seq[i] = i + 1;
    }
    for (uint64_t i = 0; i < FLAGS_waltz_key_range; i ++) {
      uint64_t r = (uint64_t)rand() % (FLAGS_waltz_key_range - i) + i;
      uint64_t temp = key_seq[i];
      key_seq[i] = key_seq[r];
      key_seq[r] = temp;
    }

    // initialize zipfian generator
    items = FLAGS_waltz_key_range;
    base = 0;
    zipf_dist = FLAGS_waltz_zipf_dist;
    theta = zipf_dist;
    zeta_2 = zeta(0, 2, 0);
    alpha = 1.0 / (1.0 - theta);
    zeta_n = zetastatic(0, FLAGS_waltz_key_range, 0);
    num_zeta = FLAGS_waltz_key_range;
    eta = (1 - pow(2.0 / items, 1 - theta)) / (1 - zeta_2 / zeta_n);
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  // WALTZ key distribution decider
  OperationType GenerateReqType(uint64_t rand_req_val) {
    if (FLAGS_waltz_test_type == 0) {
      // Microbenchmark - Uniform, write-only
      return kWrite;
    } else if (FLAGS_waltz_test_type == 1) {
      // Microbenchmark - Uniform, write:read 7:3
      return (rand_req_val>=30)?kWrite:kRead;
    } else if (FLAGS_waltz_test_type == 2) {
      // Microbenchmark - Uniform, write:read 3:7
      return (rand_req_val>=70)?kWrite:kRead;
    } else if (FLAGS_waltz_test_type == 3) {
      // Microbenchmark - Zipfian, write-only
      return kWrite;
    } else if (FLAGS_waltz_test_type == 4) {
      // Microbenchmark - Zipfian, write:read 7:3
      return (rand_req_val>=30)?kWrite:kRead;
    } else if (FLAGS_waltz_test_type == 5) {
      // Microbenchmark - Zipfian, write:read 3:7
      return (rand_req_val>=70)?kWrite:kRead;
    } else {
      return kRead; // for unknown code, run read-only
    }
  }

  // Microbenchmark for WALTZ
  void WaltzMicroBench(ThreadState *thread) {
    ReadOptions options;
    RandomGenerator gen;
    std::string value;
    int64_t reads = 0;
    ...
    bool zipfian_mode = false; // default uniform
    ...
    uint64_t key_data, scan_len = 0;

    if (FLAGS_waltz_test_type == 3 ||
        FLAGS_waltz_test_type == 4 ||
	    FLAGS_waltz_test_type == 5) { // for Zipfian microbenchmarks, set zipfian mode true
      zipfian_mode = true;
    }
    ...
    while (!duration.Done(1)) {
      ...
      // generate request
      req_type = GenerateReqType(thread->rand.Uniform(100));

      // for seek request, generate the scan length
      if (kSeek == req_type) {
        scan_len = thread->rand.Uniform(FLAGS_waltz_scan_max);
      }

      // get the zipfian value and retrieve the key from shuffle list
      if (zipfian_mode) {
        key_data = thread->shared->get_key();
      } else {
        key_data = thread->rand.Uniform(FLAGS_waltz_key_range);
      }

      // generate key
      GenerateKeyFromInt(key_data, FLAGS_waltz_key_range, &key);
      ...
      if (req_type == kSeek) {
        ...
        it->Seek(key);
        seeks ++;
        for (uint64_t scan_it = 0; scan_it < scan_len && it->Valid(); scan_it ++, it->Next()) {
          ++ seek_found;
        }
        delete it;
      ...
      if (req_type == kRead || req_type == kUpdate) {
        auto status = db->Get(options, key, &value);
        if (status.ok()) {
          ++ found;
          bytes += key_size_ + value_size;
    ...
    snprintf(msg, sizeof(msg),
        "( ops:%" PRIu64 " found:%" PRIu64 " reads:%" PRIu64 " writes:%" PRIu64 " updates:%" PRIu64 " seek:%" PRIu64 " seek_found:%" PRIu64 ")",
        FLAGS_num, found, reads, writes, updates, seeks, seek_found);
  ...
  void Run() {
    ...
    while (std::getline(benchmark_stream, name, ',')) {
      ...
      } else if (name == "enablewaltzmode") {
        waltz_mode = true;
      } else if (name == "latencybreakdown") {
        logging_enable = true;
      // [JS] Prefill for WALTZ, run fill-seq in single-threaded
      } else if (name == "prefill") {
        fresh_db = true;
        num_ = FLAGS_waltz_key_range;
        writes_ = FLAGS_waltz_key_range;
        num_threads = 1;
        method = &Benchmark::WriteSeq;
      // [JS] WALTZ benchmark
