...
IOStatus Writer::AppendRecord(const Slice& slice) {
  ...
  // ignore block_offset_, we will treat it as zero
  IOStatus s;
  bool begin = true;

  FSWritableFile *logfile = dest_->writable_file();
  size_t write_size = sysconf(_SC_PAGESIZE);
  char *buffer = (char *)spdk_zmalloc(write_size, write_size, 0, SPDK_ENV_SOCKET_ID_ANY, SPDK_MALLOC_DMA);
  //char *buffer = (char *)calloc(1, write_size);
  
  if (buffer == nullptr) {
    fprintf(stderr, "Error on spdk_zmalloc at Writer::AppendRecord\n");
  }

  do {
    const size_t avail = kBlockSize - header_size;
    ...
    // Format the header
    buffer[4] = static_cast<char>(fragment_length & 0xff);
    buffer[5] = static_cast<char>(fragment_length >> 8);
    buffer[6] = static_cast<char>(type);
    ...
    if (type < kRecyclableFullType) {
      ...
      // on the 32-bit CRC.
      EncodeFixed32(buffer + 7, static_cast<uint32_t>(log_number_));
      crc = crc32c::Extend(crc, buffer + 7, 4);
    }

    // Compute the crc of the record type and the payload.
    uint32_t payload_crc = crc32c::Value(ptr, fragment_length);
    crc = crc32c::Crc32cCombine(crc, payload_crc, fragment_length);
  ...
  logfile->Append(Slice(buffer, write_size), IOOptions(), nullptr);
  dest_->UpdateSize(write_size);
