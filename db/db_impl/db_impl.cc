...
Env *g_env = nullptr;
uint64_t write_logging[32][16]; // 8thread, 16state
thread_local int logging_idx = -1;
std::atomic<int> logging_allocator;

void prt_time(int pos) {
  if (!logging_enable)
    return;
  if (UNLIKELY(logging_idx == -1)) {
    logging_idx = logging_allocator.fetch_add(1);
  }
  if (logging_idx >= 32) {
    fprintf(stderr, "Too much logging_idx requested %d\n", logging_idx);
  }
  write_logging[logging_idx][pos] = g_env->NowMicros();
}
...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
  ...
  for (int i = 0; i < 32; i ++) {
    for (int j = 0; j < 16; j ++) {
      write_logging[i][j] = 111111;
    }
  }
  logging_allocator.store(0);
}
