...
#if 1 // debugging feature
  prt_time(0);
  auto ret = DB::Put(o, column_family, key, val);
  prt_time(15);
  if (logging_enable) {
    char buffer[1024];
    int offset = 0;
    offset += (int)sprintf(buffer, "LOG 0 ");
    int idx = logging_idx;
    for (int i = 1; i < 16; i ++) {
      offset += (int)sprintf(buffer + offset, "%lu ", (write_logging[idx][i]==111111)?111111:write_logging[idx][i] - write_logging[idx][0]);
      write_logging[idx][i] = 111111;
    }
    fprintf(stderr, "%s\n", buffer);
    write_logging[idx][0] = 111111;
  }
  return ret;
#else
...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  // [JS] retrieve code from group leader case
  Status status;
  ...
  prt_time(4);
  deadlock_protector_mutex_.Lock();
  mutex_.Lock();
  ...
  mutex_.Unlock();
  deadlock_protector_mutex_.Unlock();
  prt_time(7);

  const bool concurrent_update = two_write_queues_;
  size_t total_byte_size = WriteBatchInternal::ByteSize(my_batch);
...
// [JS] new, thread-safe, ZNS Append based WAL write path, synchronized
IOStatus DBImpl::ZnsAppendToWAL(const WriteBatch& merged_batch,
                            log::Writer* log_writer, uint64_t* log_used,
                            uint64_t* log_size) {
  assert(log_size != nullptr);
  Slice log_entry = WriteBatchInternal::Contents(&merged_batch);
  *log_size = log_entry.size();
  ...
  IOStatus io_s = log_writer->AppendRecord(log_entry);
  ...
  if (io_s.ok()) {
    ...
    stats->AddDBStats(InternalStats::kIntStatsWriteWithWal, 1);
    RecordTick(stats_, WRITE_WITH_WAL, 1);
  }
...
Status DBImpl::SwitchMemtable(ColumnFamilyData* cfd, WriteContext* context) {
  ...
  // [JS] we will keep the mutex_ until the SwitchMemtable is done
  if (!waltz_mode) {
    mutex_.Unlock();
  }
  if (creating_new_log) {
