...
int main(int argc, char **argv) {
  gflags::SetUsageMessage(std::string("\nUSAGE:\n") + argv[0] +
                  +" <command> [OPTIONS]...\nCommands: mkfs, list, ls-uuid, df, backup, restore, dump, format");
  if (argc < 2) {
  ...
  if (FLAGS_zns_pci.empty()) {
    fprintf(stderr, "You need to specify a pcie address of zns ssd using --zns_pci\n");
    return 1;
  }
  ...
  if (subcmd == "mkfs") {
    ...
    return ROCKSDB_NAMESPACE::zenfs_tool_format();
  } else {
